...
// Paid tab is excluded from New Bills page — only unpaid/partial/all shown
type TabKey = "new" | "partial" | "all";
...
export default function BillsPage({ initialData, initialTotal }: BillsClientProps) {
  const router = useRouter();
  const [data, setData] = useState<Bill[]>(() => {
    if (initialData && initialData.length > 0) return initialData as Bill[];
  ...
  const [total, setTotal] = useState(() => {
    ...
    return getInstantCache<{ total: number }>("cache:new-bills")?.total ?? 0;
  });
  ...
  const aggregate = useMemo(() => {
    ...
    for (const b of data) {
      ...
      // Skip paid bills entirely — this page is for unpaid/pending only
  ...
  useEffect(() => {
    if (
      ...
      setInstantCache("cache:new-bills", { data: initialData, total: initialTotal ?? 0 });
      setLoading(false);
  ...
  const tabs: TabKey[] = ["new", "partial", "all"];

  return (
    <div className="space-y-4">
      <PageHeader title="New Bills" description="All generated invoices & payment tracking">
        <Button size="sm" asChild>
          <Link href="/masters/new-bills">
