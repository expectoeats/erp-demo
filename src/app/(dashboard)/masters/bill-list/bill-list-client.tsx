...
interface BillListClientProps {
  initialData?: Bill[];
  initialTotal?: number;
  initialStats?: BillStats | null;
  initialFinancialYears?: FinancialYear[];
}

export default function BillListPage({ initialData, initialTotal, initialStats, initialFinancialYears }: BillListClientProps) {
  ...
  const [stats,          setStats]          = useState<BillStats | null>(initialStats ?? null);
  ...
  useEffect(() => {
    if (!(initialFinancialYears && initialFinancialYears.length > 0)) {
  ...
  useEffect(() => {
    if (
      ...
      setLoading(false);
      const t = setTimeout(() => { load(); loadStats(); }, 5000);
  ...
  const isInitialMountStats = React.useRef(true);
  useEffect(() => {
    if (
      isInitialMountStats.current &&
      initialStats &&
      !monthFilter &&
      !yearFilter &&
      !fyFilter
    ) {
      isInitialMountStats.current = false;
      return;
    }
    isInitialMountStats.current = false;
    loadStats();
