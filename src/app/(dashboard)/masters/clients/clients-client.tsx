...
export default function CustomersPage({ initialData, initialTotal }: ClientsClientProps) {
  ...
  // Instant: if server provided initialData and no search, don't refetch on mount
  const isInitialMount = React.useRef(true);
  useEffect(() => {
    if (isInitialMount.current && initialData && initialData.length > 0 && !debouncedSearch && page === 1) {
      isInitialMount.current = false;
      // cache it and schedule background revalidate after 5s
      setInstantCache("cache:clients:list", { data: initialData, total: initialTotal ?? 0 });
      setLoading(false);
