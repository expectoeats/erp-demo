...
interface ReceiptsContentProps {
  initialData?: Receipt[];
  initialTotal?: number;
  initialUnpaidBills?: BillRef[];
}

function PreselectedBillHandler({
  unpaidBills,
  setForm,
  setModalOpen,
}: {
  ...
  useEffect(() => {
    if (preselectedBillId && unpaidBills.length > 0) {
      ...
      if (b) {
        const out =
          Number(b.outstandingAmount) ||
          Number(b.grandTotal) - Number(b.paidAmount || 0);
        setForm({
          billId: b._id,
          amount: out,
          paymentMode: "upi",
  ...
  }, [preselectedBillId, unpaidBills, setForm, setModalOpen]);
...
function ReceiptsPageContent({
  ...
  const [data, setData] = useState<Receipt[]>(() => {
    if (initialData && initialData.length > 0) return initialData as Receipt[];
  ...
  const [total, setTotal] = useState(() => {
    ...
    return getInstantCache<{ total: number }>("cache:receipts:list")?.total ?? 0;
  });
  ...
  const [unpaidBills, setUnpaidBills] = useState<BillRef[]>(() => initialUnpaidBills ?? []);
  const [unpaidBillsLoading, setUnpaidBillsLoading] = useState(() => !(initialUnpaidBills && initialUnpaidBills.length > 0));
...
export default function ReceiptsClient(props: ReceiptsContentProps) {
  return <ReceiptsPageContent {...props} />;
}
